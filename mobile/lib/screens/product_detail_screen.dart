...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  Future<void> _loadRatingSummary() async {
    final summary = await _productService.getRatingSummary(widget.product.id);
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    final String fullImageUrl =
        AppConstants.resolveUrl(widget.product.imageUrl);
                          ...
                          GestureDetector(
                            ...
                            child: Row(
                              children: [
                                ...List.generate(5, (i) {
                                  final isFilled = i <
                                      (_ratingSummary?.averageRating ?? 0)
                                          .floor();
                                  return Icon(
                                    Icons.star,
                                    size: 14 * scale,
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          final favProvider = Provider.of<FavoritesProvider>(
                              context,
                              listen: false);
                          final success = await favProvider.addFavorite(
                              widget.product, size, color);
