...
class _MainPageState extends State<MainPage> {
  ...
  Widget _buildCategoriesTab({required double scale}) {
    final showSubCategories = _currentShopView == 'sub_clothes' ||
    ...
    return Navigator(
      ...
      pages: [
        MaterialPage<void>(
          key: const ValueKey('categories-root'),
          child: _buildCategoriesRootView(scale: scale),
        ),
        if (showSubCategories)
          MaterialPage<void>(
            key: const ValueKey('categories-clothes'),
            child: _buildSubClothesView(scale: scale),
          ),
        if (_currentShopView == 'category_products' &&
            _selectedCategory != null)
          MaterialPage<void>(
            key: ValueKey('category-products-${_selectedCategory!.id}'),
            child: _buildCategoryProductsView(scale: scale),
          ),
      ],
      onDidRemovePage: (_) {
        setState(() {
          if (_currentShopView == 'category_products') {
            _currentShopView =
                _selectedCategory?.parentId == null ? 'main' : 'sub_clothes';
            _selectedCategory = null;
          } else {
            _currentShopView = 'main';
          }
  ...
  Widget _buildCategoriesRootView({required double scale}) {
    const staticCategories = [
      ('New', '/images/cat_new.jpg'),
      ('Clothes', '/images/cat_clothes.jpg'),
      ('Shoes', '/images/cat_shoes.jpg'),
      ('Accessories', '/images/cat_accessories.jpg'),
    ];
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  // API categories are shown after opening Clothes.
                  ...staticCategories.map((item) {
                    final categoryName = item.$1;
                    final imgUrl = '${AppConstants.baseUrl}${item.$2}';
                    return Padding(
                      ...
                      child: _buildCategoryCard(
                        ...
                        scale,
                        () {
                          final category = _findRootCategory(categoryName);
  ...
  CategoryModel? _findRootCategory(String name) {
  ...
  void _showCategoriesLoadingMessage() {
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildSubClothesView({required double scale}) {
    ...
    final clothesCategories = _backendCategories
        .where((category) =>
            ...
            category.categoryName.toLowerCase() == 'clothes')
        .toList();
    final clothes = clothesCategories.isEmpty ? null : clothesCategories.first;
    final subCategories = clothes == null
        ? <CategoryModel>[]
        : _backendCategories
            .where((category) =>
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_isCategoriesLoading)
                    ...
                    ...subCategories.map(
                      (subCategory) => _buildSubCategoryRow(subCategory, scale),
                    ),
                ],
              ),
            ),
  ...
  Widget _buildSubCategoryRow(CategoryModel category, double scale) {
    return Container(
  ...
  String get _categoryCatalogTitle {
    final categoryName = _selectedCategory?.categoryName ?? 'Products';
    switch (_selectedGenderIndex) {
      case 0:
        return "Women's ${categoryName.toLowerCase()}";
      case 1:
        return "Men's ${categoryName.toLowerCase()}";
      default:
        return "Kids' ${categoryName.toLowerCase()}";
    }
  }
                ...
                children: [
                  ...
                  ClipRRect(
                    ...
                    child: fullImageUrl.isNotEmpty
                        ? Image.network(
                            fullImageUrl,
                            headers: AppConstants.imageHeaders(fullImageUrl),
                            width: photoW,
                            height: photoH,
                            fit: BoxFit.cover,
                            alignment: Alignment.center,
            ...
            itemBuilder: (context, index) {
              ...
              return Stack(
                ...
                children: [
                  Image.network(
                    item['image']!,
                    headers: AppConstants.imageHeaders(item['image']!),
                    fit: BoxFit.cover,
                    alignment: Alignment.center,
                    errorBuilder: (context, error, stackTrace) {
